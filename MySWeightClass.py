...
class SWeight():
    ...
    ###    pdfs:          a list of the component pdfs (these should inherit from scipy.continuous_rv)
    ...
    def __init__(self,
        ...
        print('Initialising SWeighter with the', self.method, 'method:')
        ...
        if not (len(self.yields)==len(self.pdfs)):
          raise ValueError( 'The number yields is not the same as the number of pdfs' )

        self.ncomps = len(self.yields)
        if self.method != 'roofit':
          self.pdfnorms = [ pdf.cdf(*self.discvarranges.T[1]) - pdf.cdf(*self.discvarranges.T[0]) for i, pdf in enumerate(self.pdfs) ]
          self.pdfsum   = lambda x: sum( [self.yields[i]*self.pdfs[i].pdf(x)/self.pdfnorms[i] for i in range(self.ncomps)] )
            ...
            for j, dj in enumerate(self.pdfs):
                ...
                if j<i:
                    ...
                    if self.method=='integration':
                        self.Wkl[i,j] = nquad( lambda *args: ( di.pdf(*args)/dinorm  * dj.pdf(*args)/djnorm ) / self.pdfsum(*args), self.discvarranges )[0]
                    elif self.method=='summation':
                        self.Wkl[i,j] = np.sum( ( di.pdf(*self.data.T)/dinorm * dj.pdf(*self.data.T)/djnorm ) / self.pdfsum(*self.data.T)**2 )
                    else:
                        self.Wkl[i,j] = 0.
    ...
    def nll(self, pars):
      ...
      nobs = sum(pars)
      nest = np.sum( np.log( sum( [ pars[i]*pdf.pdf(*self.data.T)/self.pdfnorms[i] for i, pdf in enumerate(self.pdfs) ] ) ) )
      return nobs - nest
        ...
        if self.method!='roofit':
            ...
            for i in range(self.ncomps):
                for j in range(self.ncomps):
                    self.intws[i,j] = nquad( lambda *args:  self.getWeight(i,*args) * self.pdfs[j].pdf(*args) / self.pdfnorms[j], self.discvarranges )[0]
    ...
    def runTSPlot(self):
        ...
        for x in self.data:
          ...
          for i, pdf in enumerate(self.pdfs):
            x_pdf = pdf.pdf(x[0])/self.pdfnorms[i]
