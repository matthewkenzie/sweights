...
# plot the weights
fig, ax = plt.subplots(1,1, figsize=(6,4))
ax.plot( sdata['mass'].to_numpy(), sdata['sw'].to_numpy(), 'r--')
ax.plot( sdata['mass'].to_numpy(), sdata['bw'].to_numpy(), 'b--')
ax.plot( sdata['mass'].to_numpy(), sdata['sw'].to_numpy() + sdata['bw'].to_numpy(), 'k-')
#ax.set_title('Raw weights')
fig.tight_layout()
fig.savefig('%s/raw_weights.pdf'%opts.dir)
fig, ax = plt.subplots(1,1, figsize=(6,4))
ax.plot(mass, sw(mass), 'r--')
ax.plot(mass, bw(mass), 'b--')
ax.plot(mass, sw(mass)+bw(mass), 'k-')
#ax.set_title('Smoothed weights')
fig.tight_layout()
