...
if opts.cow:
  ...
  sws = cw.wk(0,toy[:,0])
  bws = np.sum( [ cw.wk(i,toy[:,0]) for i in range(1,len(cw.gk)) ], axis=0 )
  ...
  #if opts.Im==3: ax.plot( ch, cw.Im(ch), 'k--', label='$I(m)$' )
  ax.plot( x, N*cw.Im(x), 'k--', label='$I(m)$' )
  ax.legend()
  ax.set_xlabel('Mass [GeV]')
  fig.tight_layout()
  fig.savefig('plots/newcowmfunc.pdf')
  ...
  fig.tight_layout()
  fig.savefig('plots/newcowweights.pdf')
...
# do the classic sweight thing
  ...
  ax.legend()
  fig.savefig('plots/newcowmfit.pdf')
  fig.tight_layout()
...
# averages for cow
if opts.cow:
  print( np.average(sws), np.var(sws)**0.5 )
  print( np.average(bws), np.var(bws)**0.5 )
...
tmi = Minuit(wnll, lb=4, limit_lb=(0,8), errordef=Minuit.LIKELIHOOD, pedantic=False)
tmi.migrad()
tmi.hesse()
print(tmi.params)
...
ax.legend()
fig.tight_layout()
fig.savefig('plots/newcowtfit.pdf')
